...
def plot_data(data_dict,joint_name=None,type=None,file_id=None):
    
    for key in data_dict[file_id]:
        if(key==joint_name):
            plt.plot(data_dict[file_id][key]['effort'])
            plt.title(f"{type} +{joint_name} + {file_id}")
            plt.show()

def initialize_dict(joint_names,file_list):
    data_dict = {}
    for file in file_list:
        file_id = os.path.basename(file).split('.')[0]
        data_dict[file_id] = {}
        for joint_name in joint_names:
            data_dict[file_id][joint_name] = {'effort':[],'position':[],'velocity':[]}
    return data_dict
