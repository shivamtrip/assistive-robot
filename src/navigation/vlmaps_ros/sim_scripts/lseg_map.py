...
def create_lseg_map_batch(img_save_dir, use_self_built_map: False ,camera_height,max_depth_value=5, cs=0.05, gs=1000, depth_sample_rate=100):
    ...
    tf_base2cam_path = os.path.join(add_params_dir, "tf_base2cam.txt")
    cam_intrinsics_path = os.path.join(add_params_dir, "cam_intrinsics.txt") 
    ...
    for idx,data_sample in enumerate(data_iter):
        ...
        tf_base2camera_optical = load_tf(tf_base2cam_path)
        ...
        pose = base_pose @ tf_base2camera_optical @ tf_camera_optical2pcd
        ...
        pix_feats = get_lseg_feat(model, rgb.copy(), labels, transform, crop_size, base_size, norm_mean, norm_std, vis)
        ...
        print("rgb_cam_mat: ", rgb_cam_mat)
        print("feat_cam_mat: ", feat_cam_mat)
